...
\textbf{Incentivized relay networks.} Nym~\cite{nym-whitepaper} and HOPR~\cite{hopr} incentivize relay operators via cryptocurrency staking and rewards. Both require blockchain infrastructure and token ownership, creating economic barriers and on-chain metadata leakage (publicly visible stake/reward transactions). Veil eliminates both by using non-transferable bilateral credits with no public ledger. The key distinction: Nym's incentive requires a globally-agreed token balance (consensus); Veil's incentive requires only local SNARK verification (no consensus).
